...
def build_examples(en_lines, vi_lines):
    ...
    for src, tgt in tqdm(zip(en_lines, vi_lines), total=len(en_lines), desc="Đang xử lý dữ liệu"):
        src = src.strip()
        tgt = tgt.strip()
        prompt = f"Translate this sentence to Vietnamese: {src}\nAnswer:"
        examples.append({"prompt": prompt, "completion": tgt})
    return examples

data = build_examples(en_lines, vi_lines)

temp_data, test_data = train_test_split(data, test_size=0.2, random_state=42)
train_data, valid_data = train_test_split(temp_data, test_size=0.125, random_state=42) 
